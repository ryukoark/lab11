...
<div class="bg-white block mt-6 p-6 border-gray-300 hover:border-indigo-300 hover:ring hover:ring-indigo-200 hover:ring-opacity-50 rounded-md shadow-sm">
    <div class="flex justify-end">
        <x-dropdown>
            ...
            <x-slot name="content">
                <x-dropdown-link :href="route('contacts.edit', ['contact' => $contact->id])">
                    Editar
                </x-dropdown-link>
                <form method="POST" action="{{ route('contacts.destroy', ['contact' => $contact->id]) }}">
                    @csrf
                    @method('delete')
                    <x-dropdown-link :href="route('contacts.destroy', ['contact' => $contact->id])" onclick="event.preventDefault(); this.closest('form').submit();">
                        Eliminar
                    </x-dropdown-link>
                </form>
            </x-slot>
